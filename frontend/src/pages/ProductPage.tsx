...
export default function ProductPage() {
  ...
  const fetchProducts = async (page = 1, pageSize = 10, search = '') => {
    setLoading(true);
    try {
      // Gọi API lấy danh sách sản phẩm với phân trang và từ khóa
  ...
  const columns = [
      ...
      render: (text: string, record: any) => (
        <div className="flex items-center gap-3">
          <Avatar 
            ...
            className="bg-neutral-100 border border-neutral-200 rounded-md"
          />
          <div>
    ...
    },
    {
      ...
      render: (gender: string) => {
        const colorMap: Record<string, string> = {
          Male: 'blue', Female: 'magenta', Unisex: 'purple', Kids: 'green'
        };
    ...
    },
    {
      ...
      render: (status: string) => {
        let color = status === 'Active' ? 'success' : status === 'Draft' ? 'warning' : 'default';
      ...
      render: (_: any, record: any) => (
        <Space size="middle">
          <Tooltip title="Chỉnh sửa sản phẩm">
            <Button 
              type="text" 
              icon={<EditOutlined className="text-neutral-500 hover:text-blue-600 transition-colors" />} 
            />
          </Tooltip>
        </Space>
      ),
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-sm text-neutral-500 mt-1 font-light">
            Xem, thêm mới và cập nhật thông tin sản phẩm, kho hàng.
          </p>
        </div>
